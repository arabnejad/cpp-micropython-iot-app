...
/** ID returned to Python for a widget that can be updated later. */
...
/** Position and size of an item on the screen, measured in pixels. */
struct Rect {
  std::int32_t x{0};
  std::int32_t y{0};
  std::int32_t width{0};
...
/** Everything needed to create one text box. */
struct TextBoxSpec {
  Rect        bounds;
  std::string text;
