 ...
 *
 * This class contains the Seesaw register details for this exact board. Code
 * that only needs normal gamepad input can use it through `GameController`.
 */
class AdafruitMiniI2cGamepad : public GameController {
public:
  /** Opens the gamepad through the normal Linux I2C implementation. */
...
private:
  ...
  std::unique_ptr<hardware::II2cDevice> gamepadI2cDevice_;
  bool                                  isConnected_{false};
  std::uint8_t                          processorHardwareId_{0};
  std::uint32_t                         productIdAndFirmwareDateCode_{0};
};
